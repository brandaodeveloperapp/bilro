...
#[cfg(test)]
mod temp_manual_bill {
    use super::*;

    #[test]
    #[ignore]
    fn manual_bill_compare() {
        let home = PathBuf::from(std::env::var("HOME").unwrap());
        let cwd = PathBuf::from("/Users/igorbrandao/Desktop/threadline");
        let always = weigh_always_on(&home.join(".claude"));
        let slug = format!("-{}", cwd.to_string_lossy().trim_start_matches('/').replace('/', "-"));
        let all = weigh_memory(&home.join(".claude").join("projects"));
        let memory: Vec<&MemoryItem> = all.iter().filter(|m| m.project == slug).collect();
        let foreign: Vec<&MemoryItem> = all.iter().filter(|m| m.project != slug).collect();
        let agents = weigh_agents(&[home.join(".claude").join("agents"), cwd.join(".claude").join("agents")]);
        let mcp = weigh_mcp(&home);
        let plugins = weigh_plugins(&home.join(".claude").join("plugins"));

        let catalogue: i64 = agents.iter().map(|a| a.catalogue_tokens).sum();
        let always_total: i64 = always.iter().map(|f| f.tokens).sum();
        let memory_total: i64 = memory.iter().map(|m| m.tokens).sum();
        let total = catalogue + always_total + memory_total;

        eprintln!("slug={slug}");
        for f in &always {
            eprintln!("always {} = {} tok", f.name, f.tokens);
        }
        for m in &memory {
            eprintln!("MEMORY.md ({} entradas) = {} tok", m.entries, m.tokens);
        }
        eprintln!("catalogo de {} agentes = {} tok", agents.len(), catalogue);
        eprintln!("TOTAL = {total} tok");
        let inherits = agents.iter().filter(|a| a.inherits_everything).count();
        let no_ctx = agents.iter().filter(|a| !a.has_context_mode).count();
        eprintln!("inherits={inherits} no_ctx={no_ctx}");
        eprintln!("mcp={} plugins={}", mcp.len(), plugins.len());
        if let Some(heaviest) = foreign.iter().max_by_key(|m| m.tokens) {
            eprintln!("foreign={} heaviest={}", foreign.len(), heaviest.tokens);
        }
    }
}
