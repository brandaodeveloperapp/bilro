 ...
 */
const SEVERE_WORDS =
  /\b(error|erro|failed|failing|failure|fails|fail|falha|falham|falhou|fatal|panic|exception|traceback|refused|denied|unauthorized|forbidden|timeout|timed out|cannot|could not|no such|not found|undefined is not|segmentation fault|FAIL)\b/i;

const SEVERE_MARKS = /[✕✗]/;

export function isSevere(line) {
  return SEVERE_WORDS.test(line) || SEVERE_MARKS.test(line);
}
...
export function rankByInformation(db, command, output, keep = 80) {
  ...
  const scored = lines.map((line, i) => {
    ...
    const idf = Math.log((n + 1) / (seen + 1)) + (isSevere(line) ? 10 : 0);
    return { i, line, idf };
  });
